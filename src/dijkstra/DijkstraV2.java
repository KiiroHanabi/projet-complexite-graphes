...
public class DijkstraV2 {
	 ...
	 * @param source
	 *            L'identifiant du noeud source de l'algorithme.
	 */
	public DijkstraV2(Graphe graphe, int source) {

		if (graphe == null) {
			System.err
					.println("Le graphe sélectionné est null. Veuillez recommencer l'opération.");
			...
			System.err
					.println("Le noeud source n'est pas présent dans le graphe.");
		}
	}
	 ...
	 * 
	 * @throws Exception
	 *             Lance une exception si la construction de la classe s'est mal
	 *             passée.
	 */
	public void start() throws Exception {
		if (g == null || ppc == null || M == null) {
			System.err
					.println("Le graphe inséré ou le noeud source ont causé une erreur, veuillez vérifier le code.");
	...
	/**
	 * Phase d'initialisation de l'algorithme, remplit la liste des noeuds non
	 * marqués en calculant leur coût et noeud père initiaux.
	 * 
	 */
	public void init() {
	...
	/**
	 * Phase itérative de l'algorithme, vérifie de le cout des successeurs et
	 * met les valeurs à jour si elles sont améliorables. Après cette phase
	...
	public void loop() {
		while (!M.isEmpty()) {
			...
			M.remove(m);
			LinkedList<Noeud> successeurs = g.getNoeuds().get(F.get(m).getId())
					.successeurs();
			for (int y = 0; y < successeurs.size(); y++) {
				if (M.containsValue(successeurs.get(y))) {
					if (ppc.get(F.get(m).getId()).getCoutPPC()
							+ cout(F.get(m).getId(), successeurs.get(y).getId()) <= ppc
							.get(successeurs.get(y).getId()).getCoutPPC()) {
						ppc.get(successeurs.get(y).getId()).setCoutPPC(
								ppc.get(F.get(m).getId()).getCoutPPC()
										+ cout(F.get(m).getId(), successeurs
												.get(y).getId()));
						ppc.get(successeurs.get(y).getId()).setPerePPC(
								g.getNoeuds().get(F.get(m).getId()));
					}
				}
			}
		}
	 ...
	 * @param fin
	 *            Le noeud de fin.
	 * @return Le coût de l'arc situé entre les deux noeuds s'il y en a un,
	 *         sinon renvoie la valeur
	 *         <strong>Double.POSITIVE_INFINITY</strong>.
	 */
	public double cout(int source, int fin) {
		if (source == fin)
			return 0;
		Arc arc = g.getNoeuds().get(source)
				.getArcByTerm(g.getNoeuds().get(fin));
		if (arc == null)
	...
	/**
	 * Détermine le noeud non marqué dont le coût est le plus petit depuis la
	 * source.
	 * 
	...
	public int selecDmin() {
		...
		for (int i = 0; i < arrayM.length; i++) {
			if (ppc.get(arrayM[i].getId()).getCoutPPC() < ppc.get(
					arrayM[min].getId()).getCoutPPC())
				min = i;
		}
	...
	/**
	 * Vérifie que le noeud demandé existe dans le graphe et appelle
	 * <i>afficherChemin()</i> pour l'afficher.
	 * 
	 * @param destination
	 *            L'identifiant du noeud dont on veut connaître le plus court
	 *            chemin depuis la source.
	 */
	public void afficherPlusCourtChemin(int destination) {
		if (ppc.containsKey(destination)) {
			System.out.print("Cout : " + ppc.get(destination).getCoutPPC()
					+ ", Chemin : " + destination + " ");
			afficherChemin(ppc.get(destination));
		} else {
			System.err
					.println("Le noeud destination n'est pas présent dans le graphe.");
		}
	}

	/**
	 * Remonte le chemin depuis le noeud choisi vers la source en l'affichant
	 * dans la console.
	 * 
	 * @param ch
	 *            Le <strong>PlusCourtChemin</strong> à remonter.
	 */
