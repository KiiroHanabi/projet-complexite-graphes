...
public class DijkstraV1Test {
	...
	public void testLoop()
	{
		...
		if(!d.getM().isEmpty())
		{
			...
			{
				assertTrue(Double.POSITIVE_INFINITY == d.getPpc().get(d.getM().get(i).getId()).getCoutPPC());
			}
		}
		else
			assertTrue(d.getM().isEmpty());
	}
	...
	{
		int m;
		int[] res = new int[]{3, 5, 2}; // Pour le graphe graph1.csv avec source = 1
		d.init();
		for(int i = 0; i < d.getM().size(); i++)
		{
			m = d.selecDmin();
